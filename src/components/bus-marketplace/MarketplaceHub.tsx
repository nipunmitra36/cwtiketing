...
const NODES: Node[] = [
  { icon: HiOutlineOfficeBuilding, title: "Bus Operators", sub: "Routes · Fleet", style: { top: "2%", left: "0%" } },
  { icon: HiOutlineDeviceMobile, title: "Passenger App", sub: "Web · Android", style: { top: "2%", right: "0%" } },
  { icon: HiOutlineUserGroup, title: "Counter Agents", sub: "POS · Walk-in", style: { bottom: "6%", left: "-2%" } },
  { icon: HiOutlineChartBar, title: "Live Analytics", sub: "Sales · Reports", style: { bottom: "6%", right: "-2%" } },
...
export default function MarketplaceHub() {
  ...
  return (
    <div
      ref={wrapRef}
      className="gsap-hero-visual relative mx-auto aspect-square w-full max-w-[440px] py-6"
    >
      <svg
        viewBox="0 0 400 400"
        className="pointer-events-none absolute inset-0 h-full w-full overflow-visible"
        role="img"
        aria-label="Flowchart of a ticketing system showing a Super Admin overseeing a bus ticketing marketplace of operators, agents, apps and payments"
      >
      ...
      <div className="absolute left-1/2 top-1/2 z-10 flex h-28 w-28 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center gap-1 rounded-3xl bg-gradient-to-br from-brand to-brand-dark text-center text-white shadow-2xl shadow-brand/40 sm:h-32 sm:w-32">
        <span className="text-[11px] font-bold uppercase leading-tight tracking-wide">Super</span>
        <span className="text-[11px] font-bold uppercase leading-tight tracking-wide">Admin</span>
      </div>
        ...
        return (
          ...
          >
            ...
            <span className="min-w-0">
              ...
              {n.sub && <span className="block truncate text-[10px] text-text-muted">{n.sub}</span>}
            </span>
          </div>
        );
      })}
